...
app.get('/api/negotiated_params', isLoggedIn, imageController.getParameters);
...
imageController.register()
  .then(() => {
    ...
    console.log("Registration Successful, starting the server...");
  })
  .catch(error => {
  ...
  .finally(() => {
    // Start the server even if registration fails
